...
\begin{enumerate}
    ...
    \item \textbf{Composition (a): fusion.} Several frozen encoders can
          ...
          full-scale multi-encoder fusion cells are sealed. The first cell, multi-scale codes plus DINOv2 features upscaled from
    ...
    \item \textbf{Composition (b): chaining.} The core concept is a
          ...
          domain router feeding six per-domain specialists on the sealed
...
capability and once by a reference execution of the sealed
...
\begin{itemize}
    ...
    \item The registry updates are filed by any party under a bond,
          ...
          key admits or extends on its own discretion: the sealed
...
\begin{itemize}
    ...
    \item The route is a weighted lottery over the top five, with
          ...
          at seal time and deterministic per anchor.
...
the sealed head computes~\cite{bunz2018}. An inner-product
...
\begin{itemize}
    ...
    \item \textbf{Disputes.} A slash is replay-gated. A filed dispute
          ...
          of sealed artifacts; the sealing discipline is what makes it
...
computed on data the contributor never saw. That number is sealed
...
sealed reference workload, publishes them with the chain, and
...
\begin{itemize}
    ...
    \item \textbf{Sealed compute environments}: Ocean Protocol's
          ...
          the challenge session's sealed scoring, where the buyer's data never
