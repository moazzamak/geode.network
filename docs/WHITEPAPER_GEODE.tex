...
\begin{abstract}
    ...
    assembly: the composed-codes architecture, i.e. frozen representation
...
to use. Innovation happens at the edges, not at the center. Anyone can
...
Anyone who does not take this path loses out. If
the best return on a capability comes from making it reusable,
self-interest and collaboration point the same direction.

One thing makes it all possible. GEODE is short for Generalized
Encoders for Open-Domain Expertise.
The idea is a registry of frozen encoders. Each maps one kind of
input, whether image, audio, text, or number series, into a code space,
...
artifacts, the Procrustes~\cite{schoenemann1966} and
CCA~\cite{hotelling1936,hardoon2004} maps, may be registered as
bridges between two blocks; each map is an exact solve, measured
...
\begin{itemize}
    ...
    \item \textbf{Replayable.} The same declared task and the same
          ...
          chain. Replay verifies consistency against the ledger;
          correspondence to work outside the ledger is enforced by the
          stakeholders who can detect and dispute it, i.e., the payer whose
          deposit is spent or the contributor whose credit is wrong can dispute it. The ledger is anchored to the public chain, so the record is
          bounded by the anchor cadence.
...
established parts under one measurement
discipline: mixture of experts, classical image codes,
closed-form fits, Bulletproofs, and microVMs. We test the economic mechanism before it ever governs
...
protocol machinery is built and measured: the registry, the
router, the probes, the ledger, and the settlement. Its rules are
shipped code with closed tests, and its numbers are held-out
...
\begin{description}
    ...
    \item[Contributor.] Anyone who registers a capability. A
          ...
          of work, and a committed claim. The kinds differ only in who
          executes them.
    ...
    \item[Reference executor.] A registered party that holds the
          frozen artifact and re-runs it inside probed sessions. It is
          ...
          contributor chooses to distribute the frozen weights. For a
    ...
    \item[Librarian.] This is the role that records credits, the one
          act that still needs a named party. The settlement around it is
          not a privilege: anyone may incorporate an entry, post a root,
          deliver a branch, claim, or resolve a challenged filing, and the
          reward follows the work. The librarian is an operator
          key, named at bootstrap by the developer's key and at maturity
          by a quorum-endorsed proposal the keyless governance executor
          carries out. The governance executor is a
          separate keyless contract with no human key; it holds that one
          power, carrying out a quorum-endorsed librarian replacement, and
          no other.
...
Every term that matters is defined here once and used consistently
throughout.
...
\begin{center}
    \begin{tabular}{@{}p{3.2cm}p{11.0cm}@{}}
        ...
        Artifact                & A capability once frozen: frozen code, committed claim,
        measured profile.                                                                 \\
        ...
        source is honest).                                                                \\
        Open probe exposure     & The window during which a session's answer is
        still subject to verification: a probe may still be drawn on it and
        the mismatch stream may still accumulate. Its length in work units
        is the detection horizon, i.e. the probed sessions the substitution
        test needs, divided by the probe rate. Accrued credits stay
        unclaimable for the exposure's duration, which is also the per-axis
...
\begin{description}
    \item[Frozen.] The system uses publisher checkpoints, fixed
          dictionaries, and closed-form heads. No component changes at
          serve time.
    ...
    \item[Measured, not asserted.] Every capability claim is scored
          on held-out data. It is committed with a content hash. It replays
    ...
    \item[Economic-only incentives.] Anti-abuse mechanisms use
          ...
          participants; identity data is not collected. Pedigree is a
          time cost, not an identity check. It is earned through the
          activation window, the activity floor, and verified work. A
          fresh key is cheap and a
    ...
    \item[No control surface.] The network has no user-selection
          ...
          pseudonymous; the metering is enforced locally at the gateway that a user
          connects to, and nothing per payer is ever written to the
          public ledger.
    \item[Earned, capped votes.] A vote counts only with pedigree: the
          activation window, the activity floor, and verified work. It
...
crypto practice, it says so in place. Two such places are paying
for access instead of identity checks, and the limits of majority
honesty.
...
\begin{enumerate}
    ...
    \item \textbf{Composition (a): fusion.} Several frozen encoders can
          ...
          without the blocks fighting each other. Both composition forms are
          measured: fusion here, chaining in the next item. Two
          full-scale multi-encoder fusion cells are committed. The first cell, multi-scale codes plus DINOv2 features upscaled from
          ...
          manifest is the fusion form that matters; alignment bridges are
          optional, measured per pair. Both outcomes are published.

    \item \textbf{Composition (b): chaining.} The core concept is a
          pipeline: capabilities link so that one stage's output becomes the
          next stage's input, and the linked stages together serve a task
          none of them serves alone, e.g. transcribe the recording and then
          read the intent out of the transcript. A chain
          $g \circ h$ is admissible exactly when the output contract of $h$
          satisfies the input contract of $g$, written
          $C_{\mathrm{out}}(h) \subseteq C_{\mathrm{in}}(g)$, i.e. the outputs
          of the first stage must be valid inputs for the second. Chaining
          is a different claim from fusion, with different failure modes:
          concatenating blocks into one head tests neither the contract
          check nor the error a first stage hands to a second.

          The form is tested end to end (Section~\ref{sec:measured}): a
          domain router feeding six per-domain specialists on the closed
          DomainNet selection. The chain reaches $0.2741$ against $0.2450$
          for the monolithic head that is the strongest single stage, and
          $0.2995$ when the router is handed the true domain. Routing
          error costs part of what specialisation earns.

          The tested scope is narrow: one chain of length
          two, in which both stages read the same frozen features. Chains of
          ...
          deep network at the current state of the art. The design does not
...
\begin{equation}
    z = f(x) \in \mathbb{R}^d.
\end{equation}
The frozen head scores the $C$ classes of the declared task as a
...
head without it). The answer is the
top-scoring class, the entry of $s$ with the largest value,
\begin{equation}
    \hat y = \argmax_{j \in [C]} s_j,
\end{equation}
Confidence is returned as a coarse bucket, never the raw
...
\end{equation}
The edges are part of the frozen artifact, so the bucketing
...
registered margin threshold, a floor that each axis publishes.
...
reported answer is exactly what the frozen head computes. It is
not cheaper than recomputing; for a linear head, nothing is
cheaper. Section~\ref{sec:proofs} prices it per axis. What it buys is
...
probabilistically. A fraction of sessions is answered twice: once by the serving
capability and once by a reference execution of the frozen
artifact. This is the shadow probe. The probed contributor pays for
...
commitment to the answer and the meter, never the answer
itself. The commitment is the answer's hash with a fresh nonce. The commitment form closes
...
opening exists only inside the closed replay environment, where a
...
Inside the network, one input walks one path. First, an input guard rejects
...
\begin{figure}[t]
    ...
    \caption{The network view. Frozen encoders (bottom row) each write
        ...
        capabilities answer tasks: arms with closed-form heads and
        primitives in microVMs. The router picks one capability per
...
Admission is by measurement, and the registry changes only
through deterministic rules. The admission rules are:
\begin{itemize}
    \item A submitted registration, an arm or a primitive, passes
          quality gates on held-out data. The claim is committed before
          evaluation. This is commit-reveal. A bad result cannot be
          re-described after the fact.
    \item Quality is measured and committed. The market prices only the
    ...
    \item Feature blocks are versioned artifacts on a code bus: each
          ...
          together.
    \item The registry updates are filed by any party under a bond,
          and a challenge escalates to the on-chain attestation quorum. No
          key admits or extends on its own discretion: the committed
          measurement or the ratified verdict decides, and the verdict is
          attested by the network's credit-holding identities, never filed
          by a single key.
\end{itemize}
Routing is deterministic
(Figure~\ref{fig:network}), and it follows these rules:
\begin{itemize}
    ...
    \item The route is a weighted lottery over the top five, with
          ...
          anchor, never by the bare artifact hash. The hash is unknowable
          at commit time and deterministic per anchor.
...
the single definition used everywhere in this paper. Abstentions are
...
$0.164$ on everything. Best-quality mode, which takes $\argmax_a s_a$,
...
The ledger is the network's hash-chained record, and the anchor
notarizes its tip on a public chain. The rules follow:

\begin{itemize}
    \item \textbf{The chain.} Every decision, be it a route, an answer, an
          abstention, or a payment event, is appended to a hash-chained
          ...
          the chain.
    \item \textbf{The librarian.} One role records credits. The
          settlement around that append is not a privilege: incorporation,
          root posting, branch delivery, claims, and the resolution of a
          challenged filing are all permissionless, and the reward follows
          the work. What is left to the role is writing the credits a
          session's evidence supports. The replay quorum's verdicts no
          longer need a filing key: the network's own credit-holding
          identities attest them on chain, and anyone finalizes. The
          librarian is an operator key,
          named at bootstrap by the developer's key and at maturity by a
          quorum-endorsed proposal the keyless governance executor carries
          out. The governance executor is a separate
          keyless contract with no human key; its only power is to file
          and press the button for that proposal, never to name the
          librarian on its own word. The role does not rotate on a
    ...
    \item \textbf{Force-inclusion queue.} Any party may post an
          ...
          contract, and the state of it is queryable on chain and is never silent.

          The reason incorporation needs no permission is that it decides
          nothing. Every step it takes is already fixed by what is on the chain:
          which entry is next, whether the deadline passed, what the bond
          returns. A rule that only one party may perform a
    ...
    \item \textbf{Executable replacement.} The librarian is
          ...
          next epoch. The tally is ON CHAIN: each eligible identity
          (vested credits above zero) attests its endorsement during the
          one-epoch notice window, weight is the vested-credit snapshot
          capped per identity at twenty percent of the eligible total, and
          the endorsement side needs two-thirds of the capped participating
          weight, a participation floor of one-third of eligible weight, and
          at least three distinct identities. The keyless governance
          executor files the proposal under a bond and presses the button
          after the timelock; the ledger executes the change only if the
          quorum endorsed exactly that target. A proposal the quorum did not
          endorse fails at the timelock and its bond burns. No caller, the
          executor included, can name the librarian on its own word: the raw
          bootstrap path is closed when the developer renounces
          ownership. The trigger stays mechanical: a recorded divergence
...
the frozen head computes~\cite{bunz2018}. An inner-product
argument is a compact certificate that a sum of products was
computed correctly. In symbols, the prover
publishes commitments to the frozen head $W$ and the input's
...
The table gives the measured cost per registered axis on the
reference CPU. Serving is a float64 mat-vec of $n = dC$ multiply-adds. Prover and
...
method, which is the model most favourable to the proof system.
...
\begin{itemize}
    ...
    \item \textbf{Disputes.} A slash is replay-gated. A filed dispute
          re-runs the frozen artifact on closed data. The re-run is itself
          ...
          proof verification. The whole apparatus rests on bit-exact replay
          of frozen artifacts; the freeze discipline is what makes it
          achievable, and the cost model assumes it, not proves it.
\end{itemize}
...
$\mathcal{L}$ the class list, i.e. what task it serves,
...
from the frozen artifact. It is never computed from a
self-description.
...
small penalty $\lambda$ on large weights. It is the same calculation
...
$\lambda$, and the training rows are part of the frozen
...
A registration is accompanied by evidence about what the artifact
computes, and the boundary of that evidence is fixed in advance.
The rules are:
...
\begin{itemize}
    \item \textbf{Contributors.} At registration the contributor
          ...
          argument~\cite{bunz2018}. It proves that the frozen head
          ...
          the frozen head produces on each sampled reference code is what
...
\begin{enumerate}
    ...
    \item The claim is committed before evaluation.
...
\textbf{What the measurement measures.} The measurement produces
one number: the artifact's held-out score on the axis metric,
computed on data the contributor never saw. That number is committed
before evaluation, replayed from the ledger, and compared against
the axis's published floor. It is never a self-report, and it is
never measured on the contributor's training data. The rules that
keep the number honest are:
\begin{itemize}
    \item Contributors train on whatever data they like. The network
          does not audit their training data, and it never scores on it.
    \item The network holds its own evaluation corpora. They are
          collected, held closed, and never released. Every submission is
    ...
    \item The developer's own bootstrap arms freeze the head's training
          rows for bit-exact replay. That is an internal discipline, not a
          requirement on contributors.
\end{itemize}

\textbf{Defenses against probing.} The evaluation data is
guarded against probing. The defenses are:
\begin{itemize}
    ...
    \item Of the axes measured in this paper only the Open Images
          ...
          standard GLUE and Speech Commands evaluation splits do not clear that minimum; SST-2 dev is $872$ rows, MNLI-matched
          dev $9{,}815$, and Speech Commands v2 test $11{,}005$. On those axes single-row
...
\textbf{Custody.} The corpora are held under a custody
regime. The rules are:
\begin{itemize}
    \item The evaluation corpora live inside a closed scoring
    ...
    \item The rubric is public. It holds the floors and metrics. Only
          the data is secret.
    \item Residual: the closed environment itself is an
          infrastructure trust point, and a corrupt majority of validators
          is outside the mechanism's reach.
\end{itemize}

\textbf{The challenge layer (overview; specified below).} The
challenge layer adds live measurement to admission. Its outline
is:
\begin{itemize}
...
submitter cannot grind it by re-rolling the commit.

\textbf{Rounds.} Challenges are drawn, not authored. They
come from a registered closed per-axis corpus under a published
...
the corpus is committed by Merkle root at axis creation. A Merkle
root is a single hash that commits to a whole set of values, so
any one of them can later be checked against it without revealing
the rest. The
...
check, never a relabeling. The audit budget pays for that
...
promise and leaves the pool. The backstop is probabilistic: a
validator is caught only where an audit re-labels a point the
artifact, and therefore the validator's own expected output, got
wrong, and the audit fraction is a registered per-axis parameter;
payment for accepted challenges rewards volume, so strictness is
enforced by detection, not assumed.
...
Failures are handled by fixed rules, so nothing is left to
discretion. The rules are:
...
\begin{itemize}
    ...
    \item \textbf{Wrong labels (human-labelled axes only).} On a
          ...
          axis root. A point that does not open is void, never
          relabeled.
    ...
    \item \textbf{Disputes.} Any party may file a dispute with a
          ...
          the frozen artifact on the challenged point inside a closed
...
the computation runs on encrypted data, so the party doing it
never sees the data. The user's device encrypts its input $z$
...
roughly $d$ queries, and the tier's claim is input privacy, i.e. the
contributor never sees the user's code or scores, not
...
arm at twenty seconds per query cannot compete on ranking against a
...
a low ranking. Each tier carries its own price floor, namely the
measured FHE reference cost for the private tier, and its own
...
frozen artifact answers once more, on the same session's input.
...
closed probe set. Each epoch a beacon-seeded fresh slice is
...
the served model \emph{is} the frozen artifact. That is the only
...
canonical CPU oracle reproduces the frozen heads bit-exactly on
...
reproduced only into the closed replay environment. The frozen
...
substitute disagrees with the frozen artifact. A crude knockoff
...
the open exposure \emph{is} the detection window, the period
during which a session's answer can still be probed and the
mismatch stream can still accumulate (Section~\ref{sec:terms}).
The security number and the payout-latency number are the same
number.
...
judge is
no cheaper. Every sampled executor must be corrupted. A host and
its executor together need a corrupt fraction of the pool raised
...
stated rather than hidden. Anyone holding the frozen artifact can register into
...
closed replay environment. The same
...
non-empty executor pool; anyone holding the frozen artifact may
...
homomorphic evaluation on the same ciphertext, so no plaintext
...
\begin{center}\small
    \begin{tabular}{@{}>{\raggedright\arraybackslash}p{2.2cm}>{\raggedright\arraybackslash}p{3.3cm}>{\raggedright\arraybackslash}p{3.5cm}>{\raggedright\arraybackslash}p{3.2cm}>{\raggedright\arraybackslash}p{2.6cm}@{}}
        \toprule
...
protocol, the probe flags, validator sampling, reference-executor
sampling, and the routing draw, derives from that beacon, composed
...
execution of the frozen artifact within the same session. The
...
genuinely lower fraction per axis). An abstention is never free. The
...
income, so any party may publish the commitment under a bond.
An unchallenged filing lands after one epoch with no privileged
party in the call. A challenge escalates to the on-chain
attestation quorum: the network's credit-holding identities
attest the verdict by the deterministic replay, weighted by
vested credits with a per-identity cap, and anyone finalizes it.
Challenges are priced against delay: a global decaying counter
makes the first challenge in a quiet period free and each further
live challenge within a twenty-one-day half-life pay a
non-refundable fee that doubles per challenge, capped at five
hundred twelve bonds, so a sustained campaign costs exponentially
more than the settlement it delays. The tally is linear in the
eligible set and is computed only when
a filing is disputed, once per dispute on the settlement rollup.
The party whose root lands earns the registered root-posting
bounty from the operations line, so the settlement
work is paid like the incorporation work: whoever did it draws
the fee. A stalled librarian cannot freeze the epoch's payments
entirely.
...
collectively \emph{capabilities}. Once frozen, either kind is
...
The catalog grows continuously. It launches with these categories:
...
contribution surface. Third-party \emph{learned representations}, such as
adapters, projections, feature maps, and alignment bridges, are
...
improvement on the closed reference workload), a measured expected
...
\begin{itemize}
    ...
    \item \textbf{Host deviations.} A host that silently modifies a
          third-party primitive serves an artifact other than the frozen
...
defenses are registered: commit-reveal submission, closed
evaluation data, replay comparison against the frozen artifact,
...
hiding the answers is impossible. The honest statement: memorization is
...
\begin{itemize}
    ...
    \item \textbf{97.5\% to attribution.} The remainder is credited
          to the contributors whose measured work served the session. A
          session that used a single artifact credits that artifact
          wholly. A chain session must divide the pool among its stages,
          and the protocol uses one rule for every such division: the
          Shapley value~\cite{shapley1953}, the standard fair way to
          split a reward among the members who jointly produced it. The
          rule works on subsets. A subset of stages has a
          \emph{coalition value}: the chain's measured end-to-end score
          with every stage outside the subset replaced by the identity
          (pass-through) stage, so the value is what the subset alone
          achieves. A stage's share is its average marginal
          contribution, the average gain in coalition value from adding
          that stage across all subsets. A stage that never improves a
          subset's value earns zero, never a subsidy. The shares of the
          paid stages always sum to the whole pool. The credit then
          releases gradually.
\end{itemize}

\textbf{Shapley is the protocol's single attribution rule.} It
applies to chains here and to representation artifacts
(Section~\ref{sec:library}). The choice is not cosmetic: on the
measured two-stage chain of Section~\ref{sec:measured}, Shapley
and the simpler leave-one-out rule, which credits each stage with
the drop in the total when that stage is removed, look at the
\emph{same} four coalition values but weight them differently,
and the two rules
give the routing stage shares that differ by a factor of $1.66$.
A protocol that named two rules would owe a session two different
amounts.

\textbf{Shapley's cost is what caps chain length.} A chain of $m$
stages has $2^m$ subsets, and every subset needs its own end-to-end
score, so the number of scores doubles with each added stage. The
protocol therefore does not compute these scores per session. It
computes them once when a chain is registered, against the chain's
closed reference workload, publishes them with the chain, and
reuses them for every session the chain serves until the chain
re-registers. Chain length is capped at four stages, which means
sixteen scores, a number a validator can replay. A chain longer
than four stages is not admissible.

The credited amounts release under fixed mechanics:
...
Measurement quorums, the admission score and probe audits, are
...
\begin{itemize}
    \item \textbf{Two questions, two answers.} Whether a vote counts
          ...
          not yet claimed, counted per behavioural identity,
          i.e. the artifact's measured response signature, so split accounts
          do not multiply weight.
    ...
    \item \textbf{Capped.} No behavioural identity may hold more
          ...
          genuinely distinct artifacts, each earning verified weight, which
          is expensive and bounded by deduplication. The cap is
    ...
    \item \textbf{Secret ballots.} Votes are committed as Pedersen
          ...
          validators opens only the weighted sums, by threshold, meaning
          a minimum number of committee members must cooperate. The
...
Registering a capability, whether an arm or a primitive, is
one procedure with these rules:
...
\begin{itemize}
    ...
    \item \textbf{The form.} Both kinds share one form: operator key,
          payout address, price per unit of work, and a committed claim. The
    ...
    \item \textbf{Deduplication.} The artifact hash is the registry
          ...
          response profile on the closed reference set. Two artifacts whose
    ...
    \item \textbf{The per-axis bond.} Alongside the fee, a
          ...
          quantity that a substitute actually arbitrages. That saving is the
          ...
          conservatism factor, an estimate that over-sizes the bond and
    ...
    \item \textbf{The challenge budget.} Each submission carries a
          ...
          contributor, never the network. The budget cannot buy less than
          the axis's registered minimum session depth: a session below the
          published floor fails closed, so a contributor cannot purchase a
          validation so thin it is meaningless.
...
\begin{center}
    \begin{tabular}{@{}p{3.4cm}p{7.6cm}@{}}
        ...
        1              & Provably fraudulent output or deviation from the frozen
...
re-run of the frozen artifact on closed data showing the
...
escalates the filing to the on-chain attestation quorum: the
network's credit-holding identities attest the verdict by the
same computation, weighted by vested credits with a per-identity
cap, and anyone finalizes it. A challenge that no quorum
substantiates within one further epoch proceeds as if
unchallenged, and the challenger loses its bond. The loser of
the dispute loses its bond, so a false accusation is not merely
wrong; it is expensive, and a conviction that survives a
challenge still lands. The bond is
...
\begin{itemize}
    ...
    \item \textbf{Appeals.} An appeal is admissible only if it
          cites at least one registered evidence class, e.g. probe mismatch
    ...
    \item \textbf{Verdict.} Let $w_v$ be validator $v$'s earned
          ...
          responders, i.e. earnings-weighted
...
way. Any party files the decision it believes was made and posts a
bond. The decision names what happened: the evaluation rule
passed, the quorum ratified, or the order authenticated. The filing sits in a
challenge window of one epoch. Unchallenged, it executes with no
privileged party in the call. Challenged, the on-chain
attestation quorum decides, exactly as for a slash; the loser of
the dispute loses its bond, and the bond is
...
false delisting would remove, or the contributor whose artifact a
false admission would expose. Refuting costs the defender
...
It must be defined, because the alternative is a quiet back channel.
...
\begin{itemize}
    ...
    \item \textbf{The nexus gate.} A freeze fires only for orders
          from a jurisdiction with registered nexus, i.e. the network
          ...
          is a procedural fact, namely incorporation records, authority
          class, and format, that the quorum decides without judging content. A
    ...
    \item \textbf{Fixed effects.} A valid order freezes: the
          ...
          equality against the frozen artifact. They never judge
    ...
    \item \textbf{Sensitive-category evidence stays
          ...
          reproduced only inside the closed replay environment and shown
...
\begin{enumerate}
    \item \textbf{Ship at 80--90\%.} Each bootstrap arm ships
          sub-maximal. The headroom is real, measured, and committed. For
    ...
    \item \textbf{The genesis council sunsets.} During the bootstrap
          epoch, governance votes run through a multi-party council of
          validators, recruited operators, and the registered bootstrap arm
          operator, never the developer alone. The council's weight is
          not staked, and it cannot route fund money to itself. It sunsets
...
\begin{itemize}
    ...
    \item \textbf{Multi-jurisdiction nexus quorum.} The compliance
          ...
          changes, including which authority keys are admitted, what
          thresholds apply, and what notice formats are used, require
          cross-jurisdiction agreement under the standard timelock. No single state dictates policy
    ...
    \item \textbf{Content-addressed releases.} The software ships by
          content hash, the IPFS identifier of the release and a
          ...
          independent parties, namely validators, gateway operators, and
          the development fund through incentivized pinning contracts, pin
...
Every role earns through a single defined line:
...
\begin{center}
    \begin{tabular}{@{}p{4.8cm}p{9.4cm}@{}}
        ...
        force-inclusion queue, held on chain as a pool that funds the
        settlement work. A fee lands in the pool when the librarian
        incorporates, or when a stranger clears the entry within the
        window; a stranger who clears a late entry earns the fee
        directly. The pool's registered draws are the root-posting
        bounty, paid to whoever's root lands, and the attestation
        reward, split among the attestors on the side a quorum verdict
        endorsed, the librarian included                                             \\
...
revenue. At the reference workload of ten thousand sessions per
...
dispute replay, one tally, and serving bookkeeping; the
root-posting bounty is a registered draw on the same line). A
base posting fee of about \$0.19 per entry closes the line at a
registered 1.25$\times$ margin; that fee is a registered floor,
not a suggestion. The line is held on chain by the settlement
contract as a pool of the posting fees, and each unit of
permissionless settlement work, e.g. posting a root, running a
batch verification, replaying a dispute, attesting a verdict, or
tallying a vote, is
paid a registered bounty from it. The payout is a pull, so no
recipient can block the work that pays it. An axis whose entry
volume falls short of the workload does not subsidise the line:
the accrual is on chain, so the shortfall is public, and an empty
pool skips a bounty publicly rather than minting one.
...
\end{equation}
the expectation over problems, i.e. the chance that at
...
rows the fit never saw. It is committed with content hashes under the
...
\begin{center}\small
    \begin{tabular}{@{}l>{\raggedright\arraybackslash}p{7.8cm}l@{}}
        \toprule
        ...
        \midrule
        \multicolumn{3}{@{}p{13.8cm}@{}}{\textit{The architecture's own cells (reproduction checks above; these are not published-checkpoint numbers)}} \\
        \midrule
...
A few readings from this table, stated plainly, follow. The vision trunk
...
router feeding specialists, on the closed DomainNet selection
(138,000 train and 34,500 held-out rows, 345 classes over 6
domains, DINOv2-small features at $56\times56$). The monolithic
head reproduces the committed read of $0.245014$ exactly, which is
...
matching that the fee rule divides over.
...
Two boundaries apply. This is one chain of length two whose stages
...
ridge head that is never fine-tuned, is measured on four modalities:
...
\emph{unqualified}, since it does not cover the declaration, and
...
fraction, which on a single-pass head is the full unit price per
axis. An abstention is never free. The registered upgrade path
...
A planted out-of-distribution (OOD) probe was measured with the
same honesty. It uses artificial inputs
...
\begin{itemize}
    ...
    \item \textbf{Classical image codes} are fixed recipes that turn an
    ...
    \item \textbf{Fixed random features} are randomized maps that
    ...
    \item \textbf{Paying by measured contribution}. Agarwal, Dahleh
          ...
          principle: closed held-out scoring, payment by measured
          out-of-sample contribution, and stake at risk. It has never
    ...
    \item \textbf{Dispute by fraud proof}: Truebit's verification
          ...
          skeleton, narrowed to a deterministic head fit: assume honest,
          challenge on objection, and settle by re-execution. The narrowing
          is what makes the bisection unnecessary.
    ...
    \item \textbf{Closed compute environments}: Ocean Protocol's
          compute-to-data~\cite{ocean2019} is the closest existing form of
          the challenge session's closed scoring, where the buyer's data never
    ...
    \item \textbf{Foundation checkpoints} are large pretrained models
    ...
    \item \textbf{Shared embedding spaces}: one joint space across
          modalities, CLIP~\cite{clip2021} for image and text and
...
the composed-codes architecture, i.e. frozen representation artifacts
...
low-rank modules at inference by gradient-free weight search, the
nearest composition relative. But the composition is
...
all; it is a scientific probe, not a production composition
...
These are the structural residuals, in order of severity:
...
\begin{enumerate}
    \item \textbf{The frozen encoder is trusted, not audited.} The
          ...
          instrument, the shadow probe, replay, dispute, and proof,
          verifies that the served model matches the frozen artifact. A
          trunk that is malicious as frozen, or backdoored by its
          publisher, passes all of them by construction. Sameness is
          certified; the frozen artifact itself is trusted, not audited.
    ...
    \item \textbf{The randomness beacon is an external dependency.}
          Every sampling guarantee in the protocol, the probe flags,
          validator sampling, reference-executor sampling, the routing
          draw, and corpus draws, reduces to the beacon. The registered
    ...
    \item \textbf{Thin axes cannot close detection inside vesting.}
          ...
          committed test statistic; no tuning can change it.
    ...
    \item \textbf{Weights-private artifacts are verified by behavioural
              identity on a fixed closed probe set}, not by fresh-input
          ...
          that differs from the frozen artifact on a small fraction of
    ...
    \item \textbf{The privacy moat and the measured capability sit on
          ...
          cheaper homomorphic evaluation, with each step measured before it
    ...
    \item \textbf{Serving substitution is detected probabilistically.} A
          ...
          and by a reference execution of the frozen artifact. The answers
          ...
          the input ciphertext and the frozen head, and the executor
          ...
          a deviation. The
          remaining residual is collusion between an executor and a
          host. It requires corrupting every sampled executor of the
          same session. That is a corrupt fraction of the
    ...
    \item \textbf{A corrupt validator's damage is bounded by the closed
    ...
    \item \textbf{Ballot secrecy rests on the tally committee.} The
          ...
          design carries.
    \item \textbf{Credit recording is a single-key act.} The
          settlement around it is permissionless: incorporation, root
          posting, branch delivery, claims, and the resolution of a
          challenged filing run with no named party. A challenged filing
          is decided by the on-chain attestation quorum: each eligible
          identity (vested credits above zero) attests whether the filing
          is wrong, weight is the vested-credit snapshot capped per
          identity at twenty percent of the eligible total, and a verdict
          needs two-thirds of the capped participating weight, a
          participation floor of one-third of eligible weight, and at
          least three distinct identities; anyone finalizes once the bar
          is met, and a challenge that reaches no verdict by the
          attestation window proceeds as if unchallenged. The one act
          that still needs a key is writing the credits a session's
          evidence supports. A stopped or captured key halts new credits;
          it cannot change a verdict, because the verdict is attested on
          chain, not filed. The failure is recorded and visible, and it
          feeds the keyless replacement discipline. One coupling is worth
          stating: the same key writes credits and, over the vesting
          window, shapes who holds the quorum's weight. A captured key can
          build allied weight within the per-identity cap; the registration
          gate, the pool bound, and the quorum-gated replacement slow that,
          and the cap is what keeps any single identity from carrying a
          verdict.
    \item \textbf{The settlement root is challenge-gated, not
              verification-gated.} The contract checks that a filed
          attribution root is not yet posted; it does not check that the
          root is the correct one for its epoch. An early, unchallenged
          wrong root can therefore land within one epoch and, because the
          root is write-once, it is then permanent. The damage is bounded:
          a claim against the wrong root still passes the artifact
          eligibility checks and can draw at most what the undrawn
          attribution pool holds. The defenses are economic: a void
          verdict burns the filer's bond, and the parties a wrong root
          would mis-pay have both the standing and the one-epoch window to
          challenge it into the attestation quorum, where their vested
          weight decides. The quorum's protection scales with the harmed
          parties' weight share: a wrong root that would mis-pay less than
          one-third of the network's eligible vested weight cannot be voided
          by the harmed parties alone, and lands by default unless the
          unaffected majority participates within the window.
    \item \textbf{A funded party can delay settlement, at a cost that
              compounds.} A challenge on any filing, even one that never
          receives a verdict, parks the settlement until the attestation
          window closes. The defense is a global, decaying challenge fee:
          the first challenge in a quiet period is free, and each further
          live challenge within a twenty-one-day half-life pays a
          non-refundable fee of one bond times two to the challenge count,
          capped at five hundred twelve bonds, on top of the refundable
          base bond. The fee funds the settlement bounties, never a
          party. The heat is global, so rotating addresses cannot reset
          it, and it decays only on twenty-one days of silence, so a
          weekly campaign never decays. What remains is a cost barrier,
          not a proof: a party with enough funds can still push a single
          epoch's settlement out by the attestation window (three days,
          down from seven) once, and a sustained denial of service is
          priced exponentially rather than removed. A challenge to a
          filing that is already resolved, already challenged, or past
          its window costs nothing and heats nothing.
    ...
    \item \textbf{A payer can grind its own route.} The lottery seed
          ...
          asked for.
    \item \textbf{Closed evaluation data invites probing.} Membership and
          ...
          submission, and rotation, push the signal below the cost of
    ...
    \item \textbf{Prices are contributor-set and can move.} Routing follows
          the posted prices within timelock bounds. Predatory cycles, undercut to capture traffic and then raise,
          are slowed and visible, not eliminated.
    ...
    \item \textbf{The validator fee schedule is registered with its
          ...
          that point the network adds a validator bond forfeitable on
...
\begin{enumerate}
    ...
    \item \textbf{Commit-reveal admission.} Submissions are committed
...
\subsection{The pools and where the money goes}

The main text states each flow where it first arises. This
section collects them into one map, so a reader can see which
pool receives which payment and where that pool's money leaves
the system. Four
holding places matter: the development fund, the attribution
pool, the operations pool, and the challenge budgets. Bonds are
held, not pooled. Figure~\ref{fig:pools} shows the routes.
...
\begin{figure}[t]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}[
                ...
                box/.style={draw, rounded corners=2pt, minimum height=8.5mm,
                        align=center, fill=blue!6},
                pool/.style={draw, rounded corners=2pt, minimum height=8.5mm,
                        align=center, fill=green!8, thick},
                arr/.style={-{Stealth}, thick}
            ]
            \node[box] (payer) at (0,3.2) {payer\\session payment};
            \node[box] (reg) at (0,1.3) {capability\\registration};
            \node[box] (post) at (0,-0.6) {queue poster\\bond + posting fee};
            \node[box] (file) at (0,-2.5) {root or slash\\filer, bond};

            \node[pool] (dev) at (5.4,3.2) {development fund\\2.5\% + registration fees};
            \node[pool] (attr) at (5.4,1.3) {attribution pool\\97.5\% of session fees};
            \node[pool] (ops) at (5.4,-0.6) {operations pool\\posting fees};
            \node[pool] (chg) at (5.4,-2.5) {challenge budget\\per submission};
            \node[pool, dashed] (bonds) at (5.4,-4.4) {bond vaults\\held, not pooled};

            \node[box] (zakat) at (10.8,3.2) {audits, tooling, security,\\public-good research;\\zakat end-state};
            \node[box] (vest) at (10.8,1.3) {credits vest\\$N=4$ epochs,\\pull claim};
            \node[box] (bounty) at (10.8,-0.6) {root-posting bounty\\to whoever's root lands};
            \node[box] (attest) at (10.8,-1.5) {attestation reward\\to the side a quorum\\verdict endorsed};
            \node[box] (valfee) at (10.8,-2.5) {validator fees\\per accepted\\challenge};
            \node[box] (refund) at (10.8,-4.4) {returned to the\\rightful party,\\else burned};
            ...
            \draw[arr] (ops) -- (attest);
            \draw[arr] (chg) -- (valfee);
            \draw[arr] (bonds) -- (refund);
        \end{tikzpicture}%
    }
    \caption{The pools and the fee routes. Each pool is a separate
        account with its own source and its own outflows; bonds are
        held, not pooled.}
    \label{fig:pools}
\end{figure}

The same map, in words, case by case:

\begin{center}
    \begin{tabular}{@{}p{6.0cm}p{9.5cm}@{}}
        \toprule
        \textbf{When this happens}              & \textbf{Where the money lands}       \\
        \midrule
        A payer buys an answer                  & the session fee splits: 2.5\% to the
        development fund and 97.5\% to the attribution pool                            \\
        A capability registers                  & the flat registration fee to the
        development fund; a per-axis bond held; a challenge budget set
        aside for its sampled validators                                               \\
        A poster uses the force-inclusion queue & the refundable bond
        held; the non-refundable posting fee to the operations pool, or
        directly to a stranger who clears the entry after its deadline                 \\
        A root or slash filing posts a bond     & the bond held, returned to
        the party the verdict vindicates, otherwise burned. A root that
        lands pays the registered root-posting bounty, and a quorum
        verdict pays the registered attestation reward to the attestors
        on the endorsed side, both drawn from the operations pool                      \\
        A contributor's work is served          & the 97.5\% credit vests over
        four epochs and is pull-claimed; a probed contributor is docked
        for the reference executor's share                                             \\
        A validator earns an accepted challenge & the per-challenge fee
        from the contributor's challenge budget, epoch-vested and
        pull-claimed                                                                   \\
        \bottomrule
    \end{tabular}
\end{center}

Two boundaries hold everywhere. The pools never exchange value
with each other: each is fed by its own source and pays its own
outflows. Bonds are never income: a bond is held, returned to
the party the rules vindicate, or burned, and it is never
collected by the network. The development fund never pays
bounties, and the operations pool's only registered draws are
the root-posting bounty and the attestation reward; the two are
not the same pot, which is the
...
\begin{enumerate}
    \item \textbf{The form.} The contributor submits the artifact
          ...
          sampled validators.
    \item \textbf{The commit.} The claim is committed before
...
cannot re-roll the commit for friendlier judges. Repeated
...
Here is the same session from a validator's seat:
...
\begin{enumerate}
    ...
    \item Sampled into an admission, it commits
          ...
          forfeitable on delisting, economic rather than identity, or
...
Buying an answer follows these steps:
...
\begin{enumerate}
    ...
    \item If the artifact abstains, the abstention is recorded and
          metered at the measured compute fraction actually consumed, the
          full unit price on a single-pass head per axis. It is never free.
\end{enumerate}
...
The honest path of a probed session runs like this:
...
\begin{enumerate}
    ...
    \item Each executor re-runs the frozen artifact on the session's
...
how often the substitute disagrees with the frozen artifact. A
...
closes; it is shipped and measured (conviction rate 0.958 within a
...
Three attempts are all closed by ordering rather than
secrecy:
\begin{itemize}
...
A dispute in which the contributor is vindicated runs like this:
...
\begin{enumerate}
    \item A mismatch is recorded against the committed answer. The
          ...
          demands a replay.
    \item The disputed input is reproduced only into the closed
          replay environment. The frozen artifact re-runs on it.
...
Becoming a reference executor takes three steps:

\begin{enumerate}
    \item A party holds the frozen artifact and registers into its
          executor pool.
...
A third-party primitive runs end to end like this:
...
primitive serves an artifact other than the frozen one. The
mismatch is replay-visible. The host faces the same slash path as
any deviation.

\subsection{The quorum takedown}

A quorum takedown runs through these steps:
...
ledger entry with no effect. The only thing an authenticated channel
can do is freeze.
...
itself; the charter has no path. It cannot mint or
receive weight.
...
Any such move is visible in the public registry. The residual, a
re-registration under a fresh address, is an identity-bound
...
inflate activity. Every payment pays the 2.5\% development-fund
dock, measured at 2.5\% of the ring's cycled credit, and
the cycled revenue buys no voting weight at all: credits earned
from a payer inside the earner's own linkage closure carry none,
so the ring spends the dock and gets neither weight nor
reputation for it, while an honest supplier keeps both. Probed
sessions pay the reference-run price on top. The gains it
...
\begin{thebibliography}{99}\small
    ...
    \emph{Biometrika}, 28(3/4):321--377, 1936.
    ...
    learning methods.
    \emph{Neural Computation}, 16(12):2639--2664, 2004.
    ...
    \bibitem{numerai}
    Numerai.
    A crowdsourced hedge fund: closed held-out scoring with staked
    predictions.
