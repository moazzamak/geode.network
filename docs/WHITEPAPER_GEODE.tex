...
none of them serves alone e.g., transcribe the recording, then read
the intent out of the transcript. A chain
...
Section~\ref{sec:proofs} prices it per axis. What it buys is
...
standard GLUE and Speech Commands evaluation splits do not; SST-2 dev is $872$ rows, MNLI-matched
...
changes, which authority keys are admitted, thresholds, and
...
skeleton, assume honest, challenge on objection, and settle by
...
full unit price on a single-pass head per axis, never free.
\end{enumerate}
